...
- `Casks/ltop.rb.template` — a **reference copy** of the live cask. It
  ...
  `pauldckim/homebrew-tap` tap repository. Both files are generated by
  `scripts/cask-update.sh` from one source (version, per-arch SHA-256,
  signing status), so the `cask "ltop" do … end` region of the two is
  byte-identical by construction and the generator self-checks it
  (`ruby -c` on both + region diff + placeholder scan). Regenerate both
  together when the cask changes — never edit one by hand
  ([../docs/DISTRIBUTION.md](../docs/DISTRIBUTION.md) §3.1).
